<template data-title="毒品定量检验记录表">
  <div class="drugsRation" v-loading='loading'>
    <el-form ref="form" :model="form" label-width="120px" size='small'>
      <div class="title-center">毒品定量检验记录表</div>
      <div class="title-level one"><i class="el-icon-tickets"></i>基本信息</div>
      <div class="form-body">
        <el-form-item class="form-items rows-really-half" label="毒品名称:">
          <el-input :disabled='isFlowShow' v-model="form.c001"></el-input>
        </el-form-item>
        <el-form-item class="form-items rows-really-half" label="受理编号:">
          <el-input v-model="form.c002" disabled></el-input>
        </el-form-item>
        <el-form-item class="form-items rows-really-half" label="检材编号:">
          <el-input v-model="form.c003" disabled></el-input>
        </el-form-item>
        <el-form-item class="form-items rows-really-half" label="检材名称:">
          <el-input :disabled='isFlowShow' v-model="form.c004"></el-input>
        </el-form-item>
      ...
      <div class="title-level one"><i class="el-icon-tickets"></i>数据记录</div>
      <div class="editor-body">
    ...
    <el-form ref="form" :model="form" label-width="80px" size='small'>
      ...
      <div class="form-body">
        ...
        <el-form-item class="form-items rows-really-half" label="检验时间:">
          <el-date-picker :disabled='isFlowShow' value-format="yyyy年MM月dd日" v-model="form.c007" type="date" placeholder="选择日期"></el-date-picker>
        </el-form-item>
      </div>
      <div class="title-level one"><i class="el-icon-tickets"></i>附表</div>
      <div class="editor-body">
        <tinymce-editor ref="editor" v-model="form.attachmentContent" ></tinymce-editor>
        <div class="tinymceMask" v-if="isFlowShow"></div>
      </div>
    </el-form>
    <div class="btns">
...
export default {
  ...
  mounted:function(){
    ...
    // 调取检材数据
    this.$set(this.form, 'c002',this.rows.bookNo);
    this.$set(this.form, 'c003',this.rows.number);
    this.$set(this.form, 'c004',this.rows.names);
    this.$set(this.form, 'checkUser',this.getData("userName"));
    this.$set(this.form, 'c007',this.timeF(new Date()).format('YYYY年MM月DD日'));

    this.$set(this.form, 'c005',Constant.drugs.inspectionReport.ration.experimentalProcess);
    this.$set(this.form, 'c006',Constant.drugs.inspectionReport.ration.analysisCondition);
    this.$set(this.form, 'attachmentContent',Constant.drugs.inspectionReport.ration.attachmentContent);
