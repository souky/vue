<template>
    <div id="entrustInfo" v-loading="loading">
            ...
            <el-row class="rowInfo">
                ...
                <el-col :span="2" :offset="18"  class="baseInfoTop">
        ...
        <el-form ref="oldAppraisalForm" label-width="150px" size="small">
            ...
            <el-row class="rowInfo">
                ...
                <div v-if=" registerInfo.oldAppraisalOrg == '1'">
                    <el-col :span="8">
            ...
            <el-row class="footerRow">
                <el-col :span="6" :offset="9">
                    <el-form-item>
                        ...
                        <el-button type="primary" @click="submitAuc()">提交鉴定</el-button>
                </el-form-item>
...
export default {
    ...
    methods: {
      ...
      addCaseInfo(){
        this.postHttp("cases/updateByPrimaryKeySelective",this.caseInfoForm,res =>{
			if(res.code == '0'){
                this.dialogVisible=false;
                this.caseInfo.caseTypeKey=this.caseInfoForm.caseTypeKey;
                this.caseInfo.caseNo=this.caseInfoForm.caseNo;
                this.caseInfo.investigationNo=this.caseInfoForm.investigationNo;
      ...
      addAttachment(){
        if(!this.attachmentText){
            this.notify_warning("请输入附件名称");
        }
        var obj={
           name: this.attachmentText,
           registerId:this.registerId
        }
        this.postHttp("evidences/saveEvidences",obj,res =>{
			if(res.code == '0'){
                this.postHttp("evidences/getEvidenceList",this.registerId,res=>{
                    this.attachmentTableData = res.data;
                });
      ...
      submitUpload() {
        ...
        this.action = this.getBaseUrl()+'evidences/saveEvidenceFile';
        ...
        let config = {
          ...
          this.fileData.append('limEvidenceId', this.limEvidenceId);
      ...
      checkAttachmentFileList(limEvidenceId){
          this.postHttp("evidencesImg/selectByEviId",limEvidenceId,res =>{
              if(res.code == '0'){
                  this.attachmentFileListDialogVisible=true;
                 this.attachmentFileListTableData=res.data;
              }
          })
      },
      deleteAttachment(rows){
          this.postHttp("evidencesImg/deleteEvidenceImgByBath",rows,res =>{
              if(res.code == '0'){
                //this.attachmentFileListDialogVisible=true;
                // this.attachmentFileListTableData=res.data;
              }
      ...
      downloadAttachment(rows){
          if(rows.length==0){
              this.notify_error("请选择下载的信息");
              return ;
          }
          if(rows.length>1){
              this.notify_error("请选择一个下载");
              return ;
          }
          var baseUrl=this.getBaseUrl();
          window.location.href=baseUrl+"evidencesImg/export?filePath="+rows[0].address+"&fileName="+rows[0].realName;
      },
      packageDownloadAttachment(rows){
          if(rows.length==0){
              this.notify_error("请选择下载的信息");
              return ;
          }
          var files='';
          for(let o in rows){
              files+=rows[o].address+",";
          }
          files=files.substring(0,files.length-1);
          var baseUrl=this.getBaseUrl();
          window.location.href=baseUrl+"evidencesImg/packageExport?urls="+files;
      },
