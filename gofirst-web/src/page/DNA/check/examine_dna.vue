...
export default{
  ...
  activated:function(){
    ...
    this.loadProcess();
    // 拿取流程过程数据
    this.setData("registerId",this.regRow.id);
  ...
  methods:{
    ...
    handleClick(tab, event) {
      ...
      if(data.model){
        ...
        if(url.indexOf(data.model+"_dna") < 0){
          this.$router.push(data.model+"_dna");
        }
      }
    },
    ...
    backList(){
      this.$router.push("appraisal_dna");
    },
    /**
     * word编辑
     */
