    ...
    <div id="authRequ">
        ...
        <el-row class="normalRow" style="margin-bottom:10px;">
            <el-col :span="22" class="normalCol">
                ...
                <span>[{{this.temp.orgName}}]送检登记</span>
...
<div class="authdiv">
        ...
        <el-form  label-width="80px" :model="authForm"  :rules="rulesForm" ref="ruleForm">
            ...
            <el-form-item label="鉴定要求:" prop="identifyRequest">
                <el-radio-group v-model="authForm.identifyRequest">
                    <el-radio v-for="item in temp.dnaTipList" :key="item.dictKey" :label="item.dictKey"  @change="requireChange(item.dictValue)">
                        {{item.dictValue}}
                    </el-radio>
                </el-radio-group>
            </el-form-item>
...
let id = 0;
  export default {
        ...
        methods: {
            ...
            getDictInfo(){
                var data = {
                    "parentKeyList":["DOCSENDTYPE","EVIDENCEUSETYPE","AUTHENTICATIONMETHOD","DNATIP"]
                };
                this.postHttpForLogin("dict/getDictInfo",data,res =>{
                    if(res.code == '0'){
                        ...
                        this.temp.dnaTipList = res.data.DNATIP;
                    }else{
             ...
             */
            backInfo(){
                this.postHttp("consignment/selectInfoByIdWhtiExt",this.authForm.consignmentId,res =>{
                    if(res.code == '0'){
            ...
            onSubmit(formName) {
                this.$refs['ruleForm'].validate((valid) => {
                 if (valid) {
                        ...
                        this.authForm.consignmentId = this.getData("consignmentId");
                        this.authForm.requireName = this.requireName;
                        let dataLet = this.unBinding(this.authForm);
                        this.postHttp("consignment/updateConsignmentById",dataLet,res =>{
                            if(res.code == '0'){
            ...
            getRegisterInfo(){
                this.postHttp("consignment/getOrgInfoByConsignmentId",this.authForm.consignmentId,res =>{
                    if(res.code == '0'){
                        //this.orgId=res.data.orgId;
                        console.log("res.data");
                        console.log(res.data);
                        this.getParentOrg(res.data.ORG_ID);
                    }else{
                        this.notify_warning(res.msg);
                    }
                });
        ...
        activated:function () {
            ...
            this.authForm.consignmentId = this.getData('consignmentId');
            this.temp.orgName = this.getData('orgName');
            this.temp.aucOrgName = this.getData('aucOrgCname');
